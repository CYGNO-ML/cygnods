...
class CygnoDataset:
    ...
    def _dataset_exists_on_path(self, path):
        ...
        self.pmt_path = self._check_folder_integrity(
            path, TIME_SERIES_FOLDER_NAME, "*.npy"
        )
        self.desc_path = self._check_folder_integrity(path, DESC_FOLDER_NAME, "*.json")
        self.particles_path = self._check_folder_integrity(
            path, PARTICLES_FOLDER_NAME, "*.txt"
        )
        self.path = path

        return bool(
            self.cmos_path and self.pmt_path and self.desc_path and self.particles_path
        )

    def load_experiment(self, experiment_id, rescaled=False):
        trajs, t_classes = self.load_experiment_trajs(
            experiment_id=experiment_id, rescaled=rescaled
        )
